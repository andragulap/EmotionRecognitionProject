...
test_mask = np.isin(actors_clean, list(test_rav) + list(test_crema))
...
history = np.load('video_history.npy', allow_pickle=True).item()
...
plt.suptitle(f'Antrenament model video  |  Test accuracy: {acc*100:.2f}%', fontsize=13)
plt.tight_layout()
...
plt.xlabel('Prezis')
plt.ylabel('Real')
plt.title(f'Confusion Matrix — Video  |  Test accuracy: {acc*100:.2f}%')
plt.tight_layout()
plt.savefig('../results/video_confusion_matrix.png', dpi=150)
plt.show()
